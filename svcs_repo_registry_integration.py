...
def auto_register_after_init(repo_path: str) -> str:
    """Auto-register repository after successful svcs init."""
    if not MANAGER_AVAILABLE:
        return "Repository manager not available - registration skipped"
    
    result = web_repository_manager.auto_register_if_initialized(repo_path)
    if result['success']:
        data = result.get('data', {})
        return f"Repository registered in central registry: {data.get('name', 'unnamed')}"
    else:
        return f"Failed to register repository: {result.get('error', 'unknown error')}"
...
def list_registered_repos():
    ...
    try:
        repositories = web_repository_manager.discover_repositories()
        return [r for r in repositories if r.get('registered', False)]
    except Exception:
        return []
